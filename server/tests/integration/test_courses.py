...
class TestCourseUpdate:
    ...
    def test_update_course(professor_client, name, is_public, is_content_public):
        course = professor_client.post("/api/v1/courses/create-course", json={"name": name, "is_public": is_public, "is_content_public": is_content_public})
    ...
    def test_update_course_forbidden_for_student(api_client, sync_sessionmaker, name, is_public, is_content_public):
        ...
        course = api_client.post("/api/v1/courses/create-course", json={"name": name, "is_public": is_public, "is_content_public": is_content_public})
        assert course.status_code == 201
        course_id = course.json()["id"]
...
class TestCourseEnrollUnenroll:
    ...
    def test_enroll_on_private_course_forbidden(api_client, sync_sessionmaker, name, is_public, is_content_public):
        ...
        api_client.post(
            "/api/v1/auth/login", json={"nickname": professor_nickname, "password": professor_password})
        create = api_client.post("/api/v1/courses/create-course", json={
            "name": name,
            "is_public": is_public,
            "is_content_public": is_content_public
        })
...
class TestChangeProfessor:
    ...
    def test_change_course_professor(api_client, sync_sessionmaker):
        ...
        login_a = api_client.post("/api/v1/auth/login", json={"nickname": professor_a_nickname, "password": professor_a_password})
        assert login_a.status_code == 200

        change = api_client.put(f"/api/v1/courses/{course_id}/change-professor", json={"new_professor_id": new_professor_id})
