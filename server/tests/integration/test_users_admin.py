...
class TestActiveApplications:
    ...
    def test_get_active_applications(api_client, sync_sessionmaker):
        ...
        api_client.post(
            "/api/v1/auth/login", json={"nickname": first_student_nickname, "password": first_student_password}
        )
        submit = api_client.post(
            f"/api/v1/users/submit-professor-application/{secret_link}", json={"name": "Ivan", "surname": "Ivanov"}
        )
        ...
        api_client.post(
            "/api/v1/auth/login", json={"nickname": second_student_nickname, "password": second_student_password}
        )
        submit = api_client.post(
            f"/api/v1/users/submit-professor-application/{secret_link}", json={"name": "Petr", "surname": "Petrov"}
        )
        assert submit.status_code == 201
        api_client.get("/api/v1/auth/logout")
        api_client.cookies.clear()
...
class TestProfessorApplication:
    ...
    def test_submit_application_success(api_client, sync_sessionmaker):
        ...
        result = api_client.post(
            f"/api/v1/users/submit-professor-application/{secret_link}", json={"name": "Иван", "surname": "Иванов"}
        )
    ...
    def test_get_my_applications(api_client, sync_sessionmaker):
        ...
        submit_result = api_client.post(
            f"/api/v1/users/submit-professor-application/{secret_link}", json={"name": "Пётр", "surname": "Петров"}
        )
    ...
    def test_approve_application_changes_role(api_client, sync_sessionmaker):
        ...
        submit_result = api_client.post(
            f"/api/v1/users/submit-professor-application/{secret_link}", json={"name": "Сергей", "surname": "Сергеев"}
        )
        assert submit_result.status_code == 201
        ...
        api_client.post("/api/v1/auth/login", json={"nickname": admin_nickname, "password": admin_password})
        approve_result = api_client.put(
            "/api/v1/users/change-application-status",
            json={"application_id": app_id, "user_id": student_id, "status": "approved"},
        )
        ...
        login_result = api_client.post(
            "/api/v1/auth/login", json={"nickname": student_nickname, "password": student_password}
        )
...
class TestAdminUserManagement:
    ...
    def test_delete_user(api_client, sync_sessionmaker):
        ...
        deleted_student = api_client.post(
            "/api/v1/auth/login", json={"nickname": student_nickname, "password": student_password}
        )
...
class TestUserProfileUpdate:
    ...
    def test_update_profile(student_client, new_nickname):
        update = student_client.put(
            "/api/v1/users/profile", json={"nickname": new_nickname, "email": f"{new_nickname}@new.com"}
        )
...
class TestUserSearch:
    ...
    def test_search_users(api_client, sync_sessionmaker):
        ...
        reg = api_client.post(
            "/api/v1/auth/reg",
            json={"nickname": nickname, "email": f"{nickname}@t.com", "password": "StrongPassword123!"},
        )
