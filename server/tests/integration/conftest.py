...
def api_client(_test_sessionmaker):
    ...
    DBDependency.db_session = lambda self: _test_sessionmaker() # noqa: ARG005
...
def _register_and_login(client, nickname, password="StrongPassword123!"):
    ...
    login_result = client.post("/api/v1/auth/login", json={"nickname": nickname, "password": password})
...
def create_admin(api_client, sync_sessionmaker, nickname="", password=""):
    ...
    assert admin_logout_result.status_code == 200, (
        f"Администратор не смог выйти из аккаунта: {admin_logout_result.text}"
    )
    api_client.cookies.clear()
...
def create_professor(api_client, sync_sessionmaker, nickname="", password=""):
    ...
    admin_login_result = api_client.post(
    ...
    submit_result = api_client.post(
        f"/api/v1/users/submit-professor-application/{secret_link}",
        json={"name": "Александр", "surname": "Евлампьев", "patronymic": "Александрович"},
    )
    ...
    professor_auth_result = api_client.post("/api/v1/auth/login", json={"nickname": nickname, "password": password})
